...
class Chessboard:
    ...
    def getValidMovesFromSquare(self, square):
        ...
        if (square[0] + move_long < self.size and square[move_short] - move_short >= 0):
            next_squares.append((square[0] + move_long, square[move_short] - move_short))
        
        # move_long left, move_short up
        if (square[0] - move_long >= 0 and square[move_short] + move_short < self.size):
            next_squares.append((square[0] - move_long, square[move_short] + move_short))

        # move_long left, move_short down
        if (square[0] - move_long >= 0 and square[move_short] - move_short >= 0):
            next_squares.append((square[0] - move_long, square[move_short] - move_short))

        # move_short left, move_long up
        if (square[0] - move_short >= 0 and square[move_short] + move_long < self.size):
            next_squares.append((square[0] - move_short, square[move_short] + move_long))

        # move_short right, move_long up
        if (square[0] + move_short < self.size and square[move_short] + move_long < self.size):
            next_squares.append((square[0] + move_short, square[move_short] + move_long))

        # move_short left, move_long down
        if (square[0] - move_short >= 0 and square[move_short] - move_long >= 0):
            next_squares.append((square[0] - move_short, square[move_short] - move_long))

        # move_short right, move_long down
        if (square[0] + move_short < self.size and square[move_short] - move_long >= 0):
            next_squares.append((square[0] + move_short, square[move_short] - move_long))
